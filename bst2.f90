...
READ*, n
! I have the first value  stored in the head and not value 0.
!Value 0 in the head may be useful for  example beacuse it makes removal of a
!value easier as you can never delete the head, but we have to check for the
!head and eliminate it from bst operations like for example in order print
READ*, k
ALLOCATE(head)
head%val = k
NULLIFY(head%left)
...
! I tested with a list with 10000 random integer values from 1 to 1000
! (generated by gl.py  in the repository
! https://github.com/beevageeva/tprog/  ) 
call removeNode(100)
call removeNode(200)
...
!could add field parent in the cell type or use a function get_parent, but in
!this case we have to search again  in the tree 
